...
async function renderTemplate(viewName: string, data: Record<string, any>): Promise<string> {
...
function getClientType(c: any): "kindle" | "modern" {
  ...
  try {
    ...
    scheduleSnap.forEach((doc) => {
      ...
      const mappedActivities = rawActivities.map((act: any) => {
        const task = (userData.tasks || []).find((t: any) => t.id === act.taskId);
...
app.get("/aac/legacy/:userId/toggle", async (c) => {
  ...
  try {
    ...
    const activity = activities.find((act: any) => act.id === itemId);
    ...
    if (isCompleted) {
      if (activity) {
        ...
        let lagStatus: "On-Time" | "A bit late" | "Late" = "On-Time";
        if (activity.endTime) {
